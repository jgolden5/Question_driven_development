  ...
  while IFS= read -r line; do
    if [[ $line == "" ]] || [[ $line == " " ]]; then
      ...
      while true ; do
        ...
        printf "\033c"
        echo "$line"
        echo "${BLACK_FG}${GREY_BG}line $line_number of $input_length ${GOLD_BG} ${percent}% ${BLUE_BG} research.txt ${NC} => y = keep, n = don't keep, b = back, j = jump, q = quit"
        read -n1 -r -s input <&3
        case $input in
					b)
						if (( line_number > 1 )); then
							echo "$input_file" | research_from_input $(( line_number - 1 ))
							break 2
						else
							echo "Cannot go back."
							sleep 0.5
